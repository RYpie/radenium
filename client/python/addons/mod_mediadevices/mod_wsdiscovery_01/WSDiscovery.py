...
NS_A_FOSCAM = "http://schemas.xmlsoap.org/ws/2004/08/addressing"
...
def parseProbeMessage(dom):
    ...
    soapHead = dom.findall('{http://www.w3.org/2003/05/soap-envelope}Header')[0]
    env.setMessageId(soapHead.findall('{http://schemas.xmlsoap.org/ws/2004/08/addressing}MessageId')[0].text)
...
class recSoaped:
    def __init__(self,dom):
        self.Header = dom.findall('{http://www.w3.org/2003/05/soap-envelope}Header')[0]
        self.Body = dom.findall('{http://www.w3.org/2003/05/soap-envelope}Body')[0]
        self.Action = self.Header.findall('{http://schemas.xmlsoap.org/ws/2004/08/addressing}Action')[0].text
        self.MessageID = self.Header.findall('{http://schemas.xmlsoap.org/ws/2004/08/addressing}MessageID')[0].text
        self.To = self.Header.findall('{http://schemas.xmlsoap.org/ws/2004/08/addressing}To')[0].text
        self.Hello = self.Body.findall('{http://schemas.xmlsoap.org/ws/2005/04/discovery}Hello')[0]
        self.EndpointReference = self.Hello.findall('{'+NS_A+'}EndpointReference')[0]
        self.Address         = self.EndpointReference.findall('{'+NS_A+'}Address')[0].text
        self.Types           = self.Hello.findall('{'+NS_D+'}Types')[0].text
        self.Scopes          = self.Hello.findall('{'+NS_D+'}Scopes')[0].text.split(' ')
        self.XAddrs          = self.Hello.findall('{'+NS_D+'}XAddrs')[0].text
        self.MetadataVersion = self.Hello.findall('{'+NS_D+'}MetadataVersion')[0].text
...
def parseHelloMessage(dom):
    ...
    env.setMessageId(dom.MessageID)
    #env.setMessageId(dom.getElementsByTagNameNS(NS_A, "MessageID")[0].firstChild.data.strip())
    env.setTo(dom.To)
    #env.setTo(dom.getElementsByTagNameNS(NS_A, "To")[0].firstChild.data.strip())
    #! Next will fail, appears no part of an onvif discovery response.
    '''
    try:
        appSeqNode = dom.getElementsByTagNameNS(NS_D, "AppSequence")[0]
        env.setInstanceId(appSeqNode.getAttribute("InstanceId"))
    ...
    env.setEPR(dom.Address)
    #env.setEPR(dom.getElementsByTagNameNS(NS_A, "Address")[0].firstChild.data.strip())
    typeNodes = dom.Types
    #typeNodes = dom.getElementsByTagNameNS(NS_D, "Types")
    if type(typeNodes) == 'list':
        env.setTypes(typeNodes[0])
        print "set typeNodes to: " + str(typeNodes[0])
    else:
        env.setTypes(typeNodes)
        
    scopeNodes = dom.Scopes
    #scopeNodes = dom.getElementsByTagNameNS(NS_D, "Scopes")
    if len(scopeNodes) > 0:
        #env.setScopes(getScopes(scopeNodes[0]))
        env.setScopes(dom.Scopes[0])
        print "set scopeNodes to: " + str(scopeNodes[0])
    else:
    ...
    if type(xNodes) == 'list':
        ...
        env.setXAddrs(xNodes[0])
        print "set xNodes to: " + str(xNodes[0])
    else:
        env.setXAddrs(xNodes)
        
    #env.setMetadataVersion(dom.getElementsByTagNameNS(NS_D, "MetadataVersion")[0].firstChild.data.strip())
    env.setMetadataVersion(dom.MetadataVersion)
    return env
...
def parseEnvelope(data):
    ...
    Zo was het eerst:
    soapHead = dom.findall('{http://www.w3.org/2003/05/soap-envelope}Header')[0]
    soapAction = soapHead.findall('{http://schemas.xmlsoap.org/ws/2004/08/addressing}Action')[0].text#.firstChild.data.strip()
    '''
    dom = recSoaped(xml)
    soapAction = dom.Action
...
if __name__ == "__main__":
    ...
    ttype1 = QName("", "Hello")
    scope1 = Scope("http://http://www.onvif.org/ver10/network/wsdl")
    ttype2 = QName("soap", "myOtherTestService_type1")
    scope2 = Scope("http://192.168.1.70:888/onvif")
    
    #xAddr = "localhost:8080/abc"
    #xAddr = "192.168.1.70:888/onvif"
    #wsd.publishService(types=[ttype], scopes=[scope2], xAddrs=[xAddr])
    #wsd.publishService(types=[ttype2], scopes=[scope2], xAddrs=[xAddr])
    #wsd.publishService(types=[ttype1], scopes=[scope1], xAddrs=[xAddr])
